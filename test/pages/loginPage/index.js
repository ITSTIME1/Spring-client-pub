...
function Q() {
  ...
  const { setAccessToken } = useAuth(); // Context API에서 accessToken과 setAccessToken 가져오기
  ...
  const sendLogin = async (event) => {
    ...
    try {
      ...
      const response = await fetch('https://www.springgreens.store/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        mode: 'cors', // 기본값이며 CORS를 활성화합니다.
