...
const Map = () => {
  ...
  const [isShowCenteredOverlay, setShowCenteredOverlay] = useState(false);

  const [isOverlapping, setIsOverlapping] = useState(false);
  const [overlayPortal, setOverlayPortal] = useState(null);
  const [storeName, setStoreName] = useState(null);
  const [setIsFirst, setFirstShow] = useState(false);
  ...
  // 비동기 거리 계산 함수
  const calculateDistanceAsync = useCallback(
    (lat1, lon1, lat2, lon2) => {
  ...
  const fetchMallStreetData = async () => {
    ...
    try {
      const response = await axios.get(
        "https://www.springgreens.store/api/map/get/mall/street",
        {
          headers: {
            "Content-Type": "application/json",
            // 필요한 경우 Authorization 헤더 등을 추가할 수 있습니다.
          },
        }
      );

      // axios는 응답이 성공적이면 response.data에 데이터를 담아 반환합니다.
  ...
  const removeAllMarkers = useCallback(() => {
    Object.values(markersRef.current).forEach(({ arriveMarker }) =>
  ...
  const setDestination = useCallback(
    async (destLat, destLng, storeName = null, width) => {
      console.log("목적지 가게 이름 : ", storeName); // 늘봄타운이 잘옴
      if (map && kakao) {
        // 기존 목적지 관련 오버레이 제거
        [destinationCircle, polyline, distanceOverlay].forEach(
          (item) => item && item.setMap(null)
        );
        ...
        const newDistanceOverlay = new kakao.maps.CustomOverlay({
          ...
          content: `<div style="padding:5px;background:transparent;border-radius:5px;color:black;">${(
            distance * 1000
          ).toFixed(0)}m</div>`,
          map: map,
        });
        ...
        const newWatchId = navigator.geolocation.watchPosition(
          async (position) => {
            ...
            const newUserPosition = new kakao.maps.LatLng(
              newUserLat,
              newUserLng
            );
            ...
            const newDistance = await calculateDistanceAsync(
              newUserLat,
              newUserLng,
              destLat,
              destLng
            ...
            const isOverlapping =
              Number((newDistance * 1000).toFixed(0)) <= Number(R) + r;
            console.log(
              "계산후 거리, 반지름의 합, 상가 반지름",
              Number((newDistance * 1000).toFixed(0)),
              Number(R) + r,
              R
            );
            console.log("overlayping ", isOverlapping);
            setIsOverlapping(isOverlapping);
            // setIsOverlayShown(true);

            newDistanceOverlay.setPosition(
              new kakao.maps.LatLng(
                (newUserLat + destLat) / 2,
                (newUserLng + destLng) / 2
              )
            );
            newDistanceOverlay.setContent(
              `<div style="padding:5px;background:transparent;border-radius:5px;color:black;">${Number(
                (newDistance * 1000).toFixed(0)
              )}m</div>`
            );
  ...
  const addStoreMarker = useCallback(
    (lat, lng, name, id) => {
      if (!map || !kakao) {
      ...
      // 도착 마커 이미지를 생성합니다
      const arriveImage = new window.kakao.maps.MarkerImage(
        arriveSrc,
        arriveSize,
        arriveOption
      );
      ...
      kakao.maps.event.addListener(arriveMarker, "click", function () {
        ...
        const renderPopup = (storeData = null, error = null) => {
          ...
          ReactDOM.render(
            <Popup
              name={name}
              onSetDestination={(lat, lng, width) =>
                setDestination(lat, lng, name, width)
              }
              storeData={storeData}
              error={error}
        ...
        fetch(
          `https://www.springgreens.store/api/map/get/products/map/${encodeURIComponent(
            name
          )}`
        )
          .then((response) => {
  ...
  useEffect(() => {
    if (map && kakao) {
      ...
      stores.forEach((store) => {
        const markerInfo = addStoreMarker(
          store.lat,
          store.lng,
          store.name,
          store.id
        ...
        for (const chunk of chunks) {
          await Promise.all(
            chunk.map((storeName, index) =>
              searchAndAddMarker(storeName, index)
            )
          );
        }
        // 지도 범위 조정
      ...
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        if (destinationCircle && destinationMarker) {
          ...
          calculateDistanceAsync(
            ...
            if (distanceOverlay) {
              distanceOverlay.setPosition(
                new kakao.maps.LatLng(
                  (newPosition.getLat() + destPosition.getLat()) / 2,
                  (newPosition.getLng() + destPosition.getLng()) / 2
                )
              );
              distanceOverlay.setContent(
                `<div style="padding:5px;background:white;border-radius:5px;color:black;">${(
                  distance * 1000
                ).toFixed(0)}m</div>`
              );
            }
  ...
  const subscribeFcmService = async () => {
    ...
    try {
      ...
      if(accessToken == null) {
        alert("토큰이 없어 액세스토큰");
        console.log("AccessToken is Null on map for subscription fcm token");
        return;
      };
      ...
      const tokenSaveResponse = await fetch(
        "https://www.springgreens.store/api/fcm/register/fcm_token",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    if (!setIsFirst) {
      if (isOverlapping) {
        ...
        if (!overlayPortal) {
          ...
          console.log("Overlay portal created and set:", portalContainer);
        } else {
          ReactDOM.render(
            <OverlayContent
              onClose={() => {
              ...
              onMove={() => {
                ...
                setTimeout(() => {
                  ...
                  console.log("이동 버튼시 storeName : ", storeName);
                  router.replace({
                    pathname: "/mainPage",
