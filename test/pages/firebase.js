...
const Firebase = () => {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      // 페이지가 로그인 상태일 때만 이곳의 코드를 실행
      console.log("User is logged in. Rendering page.");
    }
  }, [isLoggedIn]);
  ...
  useEffect(() => {
    setIsLoggedIn(true);
    // if (accessToken) {
    //   console.log("Access Token:", accessToken); // Token 확인
    //   fetch("/api/userinfo", {
    //     headers: {
    ...
    //     .then((data) => {
    //       console.log("isLoggedIn : ", true); // 상태 변경 확인
    //       setIsLoggedIn(true);
  ...
  const subscribeFcmService = async () => {
    try {
      ...
      const response = await fetch(
        "https://www.springgreens.store/api/fcm/register/fcm_service_details",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleReservation = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://www.springgreens.store/api/fcm/reserve/fcm",
        formData,
        {
          headers: {
