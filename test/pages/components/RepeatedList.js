...
const ListItem = ({ title, price, count, direction, image, onItemClick }) => {
  ...
  // 여기에서 상품증가 요청 소켓을 보내면 되겠다. 그러면 조회수 증가요청을보내니까
  const handleClick = () => {
    if (isExpanded) {
      // 화면이 확장된 상태에서 클릭되었을 때는 카운트 증가
      if (onItemClick) {
        onItemClick(); // Call the item click handler if defined
      }
    }
    // 화면이 축소된 상태에서 클릭되었을 때는 화면 확장
    ...
    return (
      <div className={styles.listBox} onClick={handleClick} style={boxStyle}>
        <div className={styles.sampleImg}>
          <Image src={image} alt="Sample Image" width={80} height={80} />
        </div>
...
const RepeatedList = ({ items = [], prevItems = [], onItemClick}) => {
